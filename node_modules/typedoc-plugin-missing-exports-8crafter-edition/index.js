...
import { Converter, ReflectionKind, TypeScript as ts, ReferenceType, ParameterType, JSX, Renderer, Application, DeclarationReflection, Context, Reflection, splitUnquotedString } from "typedoc";
...
Object.defineProperty(Reflection.prototype, "getChildByNameAndType", {
    /**
     * Return a child by its name and type.
     *
     * @this {Reflection}
     * @param {string | string[]} arg The name hierarchy of the child to look for. Can have `:kind` appended to the names to filter duplicate names based on their kind.
     * @returns {Reflection | undefined} The found child or undefined.
     */
    value: function getChildByNameAndType(arg) {
         ...
         */
        const name = names[0]?.replace(/:[a-zA-Z0-9]+$/, "");
        /**
         * @type {string | null}
         */
        const nameKindFilter = names[0]?.match(/:([a-zA-Z0-9]+)$/, "")?.[1];
        /**
         * @type {keyof typeof ReflectionKind | null}
         */
        const nameReflectionKind = nameKindFilter != null ? Object.keys(ReflectionKind).find(k=>k.toLowerCase() === nameKindFilter.toLowerCase()) ?? null : null;
        /**
        ...
        this.traverse(
            /**
             * @param {Reflection} child
             * @returns {boolean}
             */
            (child) => {
            if (child.name === name && (nameReflectionKind == null || child.kindOf(ReflectionKind[nameReflectionKind]))) {
                if (names.length <= 1) {
                    result = child;
                }
                else {
                    result = child.getChildByNameAndType(names.slice(1));
                }
                return false;
            }
            return true;
                ...
                for (const s of missing) {
                    if (shouldConvertSymbol(s, context.checker)) {
                        ...
                        if (parentOverrideTag) {
                            ...
                            let newParent = parentOverride === app.options.getValue("rootModuleParentOverrideName") ? context.project : context.project.getChildByNameAndType(parentOverride) ??
                                context.project.getChildByNameAndType(
                                    (
                                        modules.find(m =>
                                            !!context.project.getChildByNameAndType(m.name + "." + parentOverride)
                                        )?.name ?? modules.find(m =>
                                            !!context.project.getChildByNameAndType(m.escapedName + "." + parentOverride)
                                        )?.name
                                    ) + "." + parentOverride
                                );
                            if(!!!newParent) {
