...
import { Application, DeclarationReflection, ParameterType, ReflectionKind, ReflectionType, Renderer } from "typedoc";
/**
...
const svg = (color, label, ratio, width) => {
...
export function load(app) {
    ...
    app.options.addDeclaration({
        name: "coverageRefIgnoreFunctionFile",
        help: "Defines the path to a JavaScript file exporting a shouldIgnore function to be used when filtering references.",
        type: ParameterType.String,
        defaultValue: "",
    });
    app.renderer.postRenderAsyncJobs.push(async (event) => {
        /**
         * @type {{ shouldIgnore?(ref: import("typedoc").DeclarationReflection) => boolean } | null}
         */
        let filterFunction = null;
        try {
            filterFunction =
                app.options.getValue("coverageRefIgnoreFunctionFile") ?
                    await import("file://" + join(process.cwd(), app.options.getValue("coverageRefIgnoreFunctionFile")))
                :   null;
        } catch (e) {
            console.error(e);
        }
        ...
        // ========================================================================================
        let kinds = app.options.getValue("requiredToBeDocumented").reduce((acc, kindName) => acc | ReflectionKind[kindName], 0);
        ...
        outer: while (toProcess.length) {
            ...
            if (ref.kindOf(ReflectionKind.TypeLiteral) && ref.parent?.kindOf(ReflectionKind.TypeAlias)) {
            ...
            if (ref.kindOf(ReflectionKind.CallSignature) && ref.parent?.kindOf(ReflectionKind.TypeLiteral)) {
            ...
            if (ref.kindOf(ReflectionKind.ConstructorSignature) && ref.parent?.parent?.kindOf(ReflectionKind.TypeAlias)) {
                toProcess.push(ref.parent.parent);
                continue;
            }
            if (ref instanceof DeclarationReflection) {
                const signatures = ref.type instanceof ReflectionType ? ref.type.declaration.getNonIndexSignatures() : ref.getNonIndexSignatures();
            ...
            const hasComment = ref.hasComment() || (ref.kindOf(ReflectionKind.SomeSignature) && ref.parent?.hasComment());
            // Diverging from validateDocumentation here.
            if (!symbolId || symbolId.fileName.includes("node_modules")) continue;
            if (filterFunction?.shouldIgnore?.(ref)) {
                app.logger.verbose(`[typedoc-plugin-coverage]: ${ref.getFullName()} is ignored by filter function.`);
                continue;
            }
            ++expectedCount;
            if (hasComment) {
            ...
            app.logger.verbose(`[typedoc-plugin-coverage]: ${ref.getFullName()} ${ref.hasComment() ? "is" : "not"} considered documented.`);
        }
        ...
        const outFile = app.options.getValue("coverageOutputPath") || join(event.outputDirectory, "coverage.svg");
        ...
        switch (outputType) {
            ...
            default:
                ...
                app.logger.warn(`[typedoc-plugin-coverage]: Invalid coverage output type: ${outputType}`);
        }
    });
}
