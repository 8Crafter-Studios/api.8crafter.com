 ...
 */
import { JSX, ParameterType, Renderer, UrlMapping, } from "typedoc";
const default404PageContent = "404 Page Not Found";
const optionName = "page404Content";
/**
 * Add a 404 page.
 */
function add404Page(event, page404Content) {
    ...
    const page404UrlMapping = new UrlMapping("404.html", indexUrlMapping.model, (_) => {
        return JSX.createElement("div", { class: "404-content" }, JSX.createElement(JSX.Raw, { html: page404Content }));
    });
    event.urls.push(page404UrlMapping);
}
/** @ignore */
export function load(application) {
    const useHostedBaseUrlForAbsoluteLinksOptionName = "useHostedBaseUrlForAbsoluteLinks";
    ...
    // Add the 404 page.
    application.renderer.on(Renderer.EVENT_BEGIN, (event) => {
        const page404Content = application.options.getValue(optionName);
        if (typeof page404Content !== "string") {
            throw TypeError(`Unexpected ${optionName} type: ${JSON.stringify(page404Content)}`);
        }
        add404Page(event, page404Content);
        // Print the warning message here, as the option value isn't yet available before rendering.
        if (!application.options.getValue(useHostedBaseUrlForAbsoluteLinksOptionName)) {
            throw new Error(`typedoc-plugin-404 requires setting '"${useHostedBaseUrlForAbsoluteLinksOptionName}": true' to typedoc.json`);
        }
    });
