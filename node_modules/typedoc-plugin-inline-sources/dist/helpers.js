...
export function parseTag(tag, valueDeclaration) {
    const commentParts = [];
...
export function getValueDeclaration(context, reflection, sig) {
    const sym = sig
        ? context.project.getSymbolFromReflection(reflection.parent)
        : context.project.getSymbolFromReflection(reflection);
    if (!sym)
        return;
    const valueDeclaration = (sym.declarations || [])[sym.declarations ? sym.declarations.length - 1 : 0];
    return valueDeclaration;
}
function getCode(valueDeclaration) {
    const getStartPos = ts.getLineAndCharacterOfPosition(valueDeclaration.getSourceFile(), getLocationNode(valueDeclaration).getStart());
    const getEndPos = ts.getLineAndCharacterOfPosition(valueDeclaration.getSourceFile(), getLocationNode(valueDeclaration).getEnd());
...
function getLocationNode(valueDeclaration) {
