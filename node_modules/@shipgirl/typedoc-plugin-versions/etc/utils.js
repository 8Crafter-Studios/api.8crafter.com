 ...
 */
export function getMetadataPath(docRoot) {
 ...
 */
export function loadMetadata(docRoot) {
    try {
 ...
 */
export function refreshMetadata(metadata, docRoot, stable = "auto", dev = "auto", packageFile = "package.json") {
    const validate = (v) => (v === "auto" ? v : getSemanticVersion(v));
    const vStable = validate(stable);
    const vDev = validate(dev);
    const versions = refreshMetadataVersions([...(metadata.versions ?? []), metadata.stable, metadata.dev], docRoot, packageFile);
    return {
        versions,
 ...
 */
export function refreshMetadataVersions(versions, docRoot, packageFile) {
    return (
        [
            // metadata versions
            ...versions
                ...
                .filter((version) => {
                    try {
                        ...
                        return fs.pathExistsSync(vPath) && fs.statSync(vPath).isDirectory() && semver.valid(version, true) !== null;
                    } catch {
 ...
 */
export function refreshMetadataAlias(alias, versions, stable = "auto", dev = "auto") {
    ...
    if (
        ...
        if (
            ...
            (alias !== "dev" || // when alias is dev, only return latest dev if >= latest stable
                ...
                semver.gte(latest, getLatestVersion("stable", versions, stable, dev), true))
        ) {
 ...
 * @param docRoot The path to the docs root.
 */
export function saveMetadata(metadata, docRoot) {
 ...
 */
export function getLatestVersion(alias, versions, stable = "auto", dev = "auto") {
    return [...versions].sort(semver.rcompare).find((v) => getVersionAlias(v, stable, dev) === alias);
}
/**
 ...
 */
export function getVersionAlias(version, stable = "auto", dev = "auto") {
 ...
 */
export function getSemanticVersion(version) {
    if (!version) {
    ...
    return prerelease ? `v${semVer.version}-${semver.prerelease(version, true).join(".")}` : `v${semVer.version}`;
}
/**
 ...
 */
export function getPackageDirectories(docRoot) {
    return fs.readdirSync(docRoot).filter((file) => {
        const filePath = path.join(docRoot, file);
        return fs.pathExistsSync(filePath) && fs.statSync(filePath).isDirectory() && semver.valid(file, true) !== null;
    });
}
/**
 ...
 */
export function getVersions(directories) {
    return directories.filter((dir) => semver.coerce(dir, { loose: true })).map((dir) => getSemanticVersion(dir));
}
/**
 ...
 * @returns
 */
export function makeJsKeys(metadata) {
    ...
    const lines = ['"use strict"', "export const DOC_VERSIONS = [", ...keys.map((v) => `	'${v}',`), "];"];
    return lines.join("\n").concat("\n");
}
/**
 ...
 * @param pegVersion
 */
export function makeAliasLink(alias, docRoot, pegVersion, makeRelativeSymlinks) {
    ...
    if (!fs.pathExistsSync(stableSource)) throw new Error(`Document directory does not exist: ${pegVersion}`);
    if (fs.lstatSync(stableTarget, { throwIfNoEntry: false })?.isSymbolicLink()) fs.unlinkSync(stableTarget);
 ...
 * @param dev
 */
export function makeMinorVersionLinks(versions, docRoot, makeRelativeSymlinks, stable = "auto", dev = "auto") {
    for (const version of versions
        ...
        .map((version) => {
            ...
            const highestStablePatch = versions.find(
                (v) =>
                    ...
                    semver.satisfies(v, `${semver.major(version)}.${semver.minor(version)}.x`, { includePrerelease: true })
            );
            // fallback to highest patch
            return (
                highestStablePatch ??
                versions.find((v) => semver.satisfies(v, `${semver.major(version)}.${semver.minor(version)}.x`, { includePrerelease: true }))
            );
        })
        ...
        if (makeRelativeSymlinks) process.chdir(docRoot);
        if (fs.lstatSync(target, { throwIfNoEntry: false })?.isSymbolicLink()) fs.unlinkSync(target);
 ...
 */
export function getSymlinkVersion(symlink, docRoot) {
    const symlinkPath = path.join(docRoot, symlink);
    if (fs.pathExistsSync(symlinkPath) && fs.lstatSync(symlinkPath).isSymbolicLink()) {
        const targetPath = fs.readlinkSync(symlinkPath);
        if (fs.pathExistsSync(targetPath) && fs.statSync(targetPath).isDirectory()) {
 ...
 * @returns the paths
 */
export function getPaths(app, version) {
 ...
 * @param targetPath
 */
export function handleJeckyll(rootPath, targetPath) {
 ...
 * @param targetPath
 */
export function handleAssets(targetPath, srcDir = path.dirname(fileURLToPath(import.meta.url))) {
...
export function getPackageVersion(packageFile) {
