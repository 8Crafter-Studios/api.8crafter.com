 ...
 * @module
 */
import { Application, ParameterType, RendererEvent, Converter } from "typedoc";
import path from "path";
import fs from "fs-extra";
import * as vUtils from "./etc/utils.js";
import * as vHooks from "./etc/hooks.js";
import { getPackageVersion } from "@shipgirl/typedoc-plugin-versions/etc/utils.js";
...
export function load(app) {
    ...
    app.on("bootstrapEnd", (instance) => {
        const vOptions = app.options.getValue("versions");
        vHooks.injectSelectJs(app);
        vHooks.injectSelectHtml(app, vOptions.domLocation);
        const { rootPath, targetPath } = vUtils.getPaths(app, getPackageVersion(vOptions.packageFile));
        console.log(rootPath, targetPath);
        /**
        ...
        app.renderer.on(RendererEvent.END, () => {
            ...
            const metadata = vUtils.refreshMetadata(vUtils.loadMetadata(rootPath), rootPath, vOptions.stable, vOptions.dev, vOptions.packageFile);
            vUtils.makeAliasLink("stable", rootPath, metadata.stable ?? metadata.dev, vOptions.makeRelativeLinks);
            vUtils.makeAliasLink("dev", rootPath, metadata.dev ?? metadata.stable, vOptions.makeRelativeLinks);
            vUtils.makeMinorVersionLinks(metadata.versions, rootPath, vOptions.makeRelativeLinks);
            const jsVersionKeys = vUtils.makeJsKeys(metadata);
            fs.writeFileSync(path.join(rootPath, "versions.js"), jsVersionKeys);
            fs.writeFileSync(path.join(rootPath, "index.html"), `<meta http-equiv="refresh" content="0; url=${metadata.stable ? "stable/" : "dev/"}"/>`);
